...
const translations = {
  ...
  pl: {
    ...
    'routine.emptyBody': 'Dodaj produkty do półki i przypisz je do rutyny, aby zacząć.',
    ...
    'settings.deleteAllDataBody': 'To trwale usunie wszystkie produkty, rutyny i historię. Tej akcji nie można cofnąć.',
    'settings.detailedExplanations': 'Szczegółowe wyjaśnienia',
    'settings.detailedExplanationsBody': 'Pokazuj, dlaczego składniki się wykluczają. Ostrzeżenia są zawsze widoczne.',
    'settings.developer': 'Deweloper',
    'settings.exportData': 'Eksport danych',
    'settings.exportFailed': 'Eksport nieudany',
    'settings.exportFailedBody': 'Nie można wyeksportować danych. Spróbuj ponownie.',
    ...
    'settings.rateBody': 'Dziękujemy za zainteresowanie. Ocena jest dostępna w aplikacjach mobilnych.',
    ...
    'shelf.emptyBody': 'Dodaj produkty pielęgnacyjne, aby śledzić daty ważności i budować rutynę.',
  ...
  ua: {
    ...
    'routine.emptyBody': 'Додайте продукти на полицю та призначте їх до рутини, щоб почати.',
    ...
    'settings.deleteAllDataBody': 'Це назавжди видалить усі продукти, рутини та історію. Цю дію не можна скасувати.',
    'settings.detailedExplanations': 'Детальні пояснення',
    'settings.detailedExplanationsBody': 'Показувати, чому інгредієнти конфліктують. Попередження завжди видимі.',
    'settings.developer': 'Розробник',
    'settings.exportData': 'Експорт даних',
    'settings.exportFailed': 'Помилка експорту',
    'settings.exportFailedBody': 'Не вдалося експортувати дані. Спробуйте ще раз.',
    ...
    'settings.rateBody': 'Дякуємо за інтерес. Оцінювання доступне в мобільних застосунках.',
    ...
    'shelf.emptyBody': 'Додайте продукти догляду, щоб відстежувати терміни придатності та будувати рутину.',
...
const normalizeLocale = (value?: string | null): AppLocale | null => {
  ...
  if (SUPPORTED_LOCALES.includes(language as AppLocale)) return language as AppLocale;
...
export const [IntlProvider, useIntl] = createContextHook(() => {
  ...
  const t = useCallback((key: TranslationKey, params?: TranslationParams) => {
    return interpolate(translations[locale][key] ?? translations.en[key], params);
  }, [locale]);

  const dateFormatters = useMemo(() => ({
    dayMonth: new Intl.DateTimeFormat(intlLocaleTags[locale], dateFormatOptions.dayMonth),
    longDayMonth: new Intl.DateTimeFormat(intlLocaleTags[locale], dateFormatOptions.longDayMonth),
    monthYear: new Intl.DateTimeFormat(intlLocaleTags[locale], dateFormatOptions.monthYear),
    productDate: new Intl.DateTimeFormat(intlLocaleTags[locale], dateFormatOptions.productDate),
    weekdayShort: new Intl.DateTimeFormat(intlLocaleTags[locale], dateFormatOptions.weekdayShort),
  }), [locale]);

  const formatDate = useCallback((date: Date, formatKey: DateFormatKey) => {
