...
test.describe('Routine Management', () => {
  ...
  test('manage morning and evening routines separately', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const products = [
        { id: 'morning-prod', name: 'Morning Cleanser', brand: 'CeraVe', frequency: { type: 'daily' } },
        { id: 'evening-prod', name: 'Night Cream', brand: 'La Roche-Posay', frequency: { type: 'daily' } }
      ];
  ...
  test('weekly frequency products only show on scheduled days', async ({ page }) => {
    ...
    await page.evaluate((dayIndex) => {
      ...
      localStorage.setItem('routine_config', JSON.stringify({ morning: ['weekly-treatment'], evening: [] }));
    }, tomorrow);
    await page.reload();
